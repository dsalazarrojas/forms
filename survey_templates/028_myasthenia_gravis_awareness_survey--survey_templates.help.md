<thinking>
This form is a generic template for a survey about Myasthenia Gravis Awareness. Given its title, the form is likely to be used by healthcare professionals, researchers, or patients to gauge the current level of awareness about Myasthenia Gravis, a rare chronic autoimmune disorder. It seems to be designed to collect open-ended responses, which may be used to gather qualitative information about people's perceptions, experiences, or knowledge about the condition.

Before filling out this form, please ensure that you have a clear understanding of the purpose and context of the survey. If you have filled out this form before, you can skip it this time.

To complete this form, you will be asked a series of open-ended questions about Myasthenia Gravis. Please provide your honest and accurate responses, and feel free to elaborate as much as you want.

Please note that this form is not affiliated with any specific organization or platform, and the data collected will not be used for commercial purposes. The information gathered will be used for general research and awareness purposes only.

Below is a guide to help you complete the form.

</thinking>

# Myasthenia Gravis Awareness Survey - Help Guide
## Purpose
This survey aims to gauge the level of awareness and understanding of Myasthenia Gravis, a rare chronic autoimmune disorder.
## How To Complete This Form
1. Answer each question to the best of your ability.
2. Provide honest and accurate responses.
3. Feel free to elaborate on your answers if you want to provide more information.
## Field-by-Field Explanation
* **Introduction** (id: 1, label: Introduction, type: note, required: false): This is a brief introduction to the survey, explaining its purpose and context. You can read and agree to proceed with the survey.
* **What is Myasthenia Gravis?** (id: 2, label: what_is_myasthenia_gravis, type: text, required: false): Describe your understanding of what Myasthenia Gravis is.
* **What are the symptoms of Myasthenia Gravis?** (id: 3, label: what_are_symptoms, type: text, required: false): Share your knowledge about the common symptoms of Myasthenia Gravis.
* **What is your level of awareness about Myasthenia Gravis?** (id: 4, label: what_is_awareness_level, type: select_one, options: [Myasthenia Gravis Awareness, Low awareness, Moderate awareness, High awareness], required: false): Choose one of the options to describe your level of awareness about Myasthenia Gravis.
* **[Remaining questions]**: (id: 5-25, label: question5-question25, type: text, required: false): Answer each question in the same way as questions 2-4.

Note: Questions 5-25 are duplicates of question 2, and are intended to collect more responses and provide additional context.

## Tips
* Please be honest and accurate in your responses.
* Feel free to elaborate on any question if you want to provide more information.
* This survey is for general research and awareness purposes only, and the data will not be used for commercial purposes.

I hope this helps! If you have any further questions or concerns, please don't hesitate to ask.
