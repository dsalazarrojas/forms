<thinking>As the form designer, I need to ensure that the purpose and instructions for the "order_cancellation_refund_form" are clear and concise for end users. This form is used to process order cancellations and refunds, so it's essential to outline the steps for users to follow when filling out the form. The form collects information about the order cancellation reason, date, and status, as well as details about the refund, customer service response, and customer contact information. It's crucial to understand that the form fields are categorized into different sections, with some fields required for the process to be completed successfully. The user needs to provide a valid reason for cancellation, select the correct order status, and enter the refund amount and method. Additionally, the user must enter the customer's name and contact information, including email and phone number, if provided. This form aims to streamline the cancellation and refund process, ensuring that all necessary details are captured for efficient processing.</thinking>
