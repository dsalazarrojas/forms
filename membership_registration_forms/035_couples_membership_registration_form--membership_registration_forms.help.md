# Couples Membership Registration Form - Help Guide
## Purpose
This form is designed to collect information from couples interested in joining a membership program.

## How To Complete This Form
To complete this form, please follow the steps below:
1. Fill in your first and last name.
2. Enter your email address.
3. Provide your address.

## Field-by-Field Explanation
* **First Name**: Please enter your first name.
* **Last Name**: Please enter your last name.
* **Email**: Enter your email address. Please use a valid email format (e.g., `john.doe@example.com`).
* **Address**: Please provide your address where you would like to receive membership benefits and updates.

## Tips
* Please ensure that you fill out the form accurately and completely.
* If you have any questions or concerns, feel free to contact `form@gic.mx`.
