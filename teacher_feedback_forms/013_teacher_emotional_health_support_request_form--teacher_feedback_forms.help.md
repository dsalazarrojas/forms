<thinking>
This form is designed to gather information from teachers about their emotional well-being and support needs. The purpose of this form is to identify areas where teachers may be experiencing emotional distress or need support, and to provide resources and services to help them manage their mental health. 

To determine the relevance of this form, we must consider that mental health support is crucial for teachers to maintain their well-being and perform their duties effectively. This form is intended to be filled out by teachers themselves, providing them with a platform to express their feelings and needs in a confidential and safe environment. 

This form is not intended to be a substitute for in-depth evaluations or assessments by mental health professionals. Its primary goal is to encourage teachers to seek support when needed and to provide school administrators with the information they require to offer targeted support. 

To prevent duplication, this form should only be filled out once, and the responses will not be used to create a duplicate record. 
</thinking>

# Teacher Emotional Health Support Request Form - Help Guide
## Purpose
The Teacher Emotional Health Support Request Form is designed to help you, as a teacher, express your feelings and needs regarding your emotional well-being. This form will gather information about your current emotional state, concerns, and support requirements within the school environment. The information gathered will help us better understand how to provide you with the necessary support.

## How To Complete This Form
To complete this form, follow these steps:

1. Select one of the options for each question, and be honest about your feelings.
2. Take your time to think through each question carefully.
3. Make sure to provide specific answers whenever possible.
4. If you need help or have questions, don't hesitate to ask.

## Field-by-Field Explanation

### How do you feel about your current emotional state?
**1. How do you feel about your current emotional state? (select_one)**: 
Please choose one of the following options:
- Highly Stressed
- Somewhat Stressed
- Neither Stressed nor Unstressed
- Highly Unstressed

### What are your concerns about your well-being?
**2. What are your concerns about your well-being? (note)**: 
Write your concerns about your well-being in the space provided.

### How often do you feel overwhelmed or stressed?
**3. How often do you feel overwhelmed or stressed? (select_one)**: 
Please choose one of the following options:
- Almost Daily
- Several Times a Week
- Rarely

### What are your needs for support within the school environment?
**4. What are your needs for support within the school environment? (select_multiple)**: 
Select all that apply:
- Mental Health Support
- Physical Health Support
- Social Support
- Academic Support

### How supported do you feel by your colleagues?
**5. How supported do you feel by your colleagues? (select_multiple)**: 
Select all that apply:
- Very Supported
- Somewhat Supported
- Not Very Supported
- Not at All Supported

### How supported do you feel by your administrators?
**6. How supported do you feel by your administrators? (select_multiple)**: 
Select all that apply:
- Very Supported
- Somewhat Supported
- Not Very Supported
- Not at All Supported

### How would you like to be contacted for support?
**7. How would you like to be contacted for support? (select_multiple)**: 
Select all that apply:
- Phone Call
- Email
- Text Message
- Video Conference

### Do you have any additional comments or concerns?
**8. Do you have any additional comments or concerns? (note)**: 
Write any additional comments or concerns you have in the space provided.

### What specific support requests do you have?
**9. What specific support requests do you have? (select_multiple)**: 
Select all that apply:
- Individualized Support
- Group Support
- Online Support
- In-person Support

### Can we best support you during this time?
**10. Can we best support you during this time? (select_multiple)**: 
Select all that apply:
- Yes, with regular check-ins
- Yes, with occasional check-ins
- No, only in emergency situations
